...
- 整个页面已升级为五段式叙事结构进行“今天 -> 宏观 -> 旅程 -> 周期 -> 微观”的降维展现：
  - **Poster Hero**：今日 Rings + Bento Grid，并在底部插入“历史上的今天”时间回声条。
  - **Macro Overview**：2x2 胖卡片统计叠加可切换的年份热力图 / Fitness-Fatigue 状态曲线。
  - **Voyage Section**：把累计里程映射为真实世界航线的虚拟远征模块。
  - **Trend Section**：基于周一算起的本周/本月/本年的 1x4 指标流体切换排布。
  - **Dashboard Details**：附带深邃左侧高亮选择态、训练 DNA 指纹条、右侧微观图表与里程碑时间线。
- 引入了基于原始时序数据和历史记录的轻量分析层，用于生成连续周数、训练里程碑、Fitness-Fatigue 状态、训练 DNA、虚拟远征进度和“历史上的今天”对比。
...
- 所有被提炼出来的页面分区和高频交互件：
  ...
  - `src/components/voyage-section.tsx` (新增：虚拟远征航线模块，用抽象 SVG path 映射累计里程)
  - `src/components/trend-section.tsx` (阶段聚焦容器：管理周、月、年的跨度)
  - `src/components/dashboard-section.tsx` (深度数据解剖容器：历史列表、训练 DNA 指纹、右侧微观图表与里程碑时间线)
...
- 数据向业务 View 转换的桥梁。把原始杂乱的数据组装给大卡片。当前除原有模块外，已新增多个纯分析 / 视图模型模块：
  ...
  - `voyage-data.ts`（虚拟远征路线与进度）
  - `dna-data.ts`（训练 DNA 指纹与节奏类型分类）
...
#### `VoyageSection`
- 新增的中层叙事模块。它把累计总里程映射为预定义真实航线（如西湖、英吉利海峡、长江三峡段、泰晤士河等）的分段远征。
- UI 采用抽象 SVG path + 已完成段发光描边 + 当前进度脉冲光标，避免引入真实地图 SDK，保持极简与可控。
- 路线切换完全由 `totalDistance` 驱动，不引入任何额外持久化状态。
...
- 历史列表每一行底部新增了 `训练 DNA` 指纹条：基于 `paceList` 拆解出的颜色段，配合 `匀速型 / 间歇型 / 渐入佳境 / 前快后慢` 标签，用于快速识别训练节奏。
...
- `time-machine-data.ts`、`milestones-data.ts`、`fitness-fatigue-data.ts`、`voyage-data.ts`、`dna-data.ts` 均为纯 view-model 层，不直接执行 I/O。
- `app/page.tsx` 会在服务端同时拿到 `recentRecords` 与 `allHistoryRecords`：
  - `recentRecords` 用于当前页面的历史列表和默认详情。
  - `allHistoryRecords` 用于 Time Machine、连续周数、里程碑、Fitness-Fatigue 和 DNA 分析，避免分析只基于最近记录而失真。
...
### 4.1 首页 `/`
- 当前首页结构已演化为：`PosterHero(+TimeMachine)` -> `MacroOverview(+Heatmap/Fitness 状态)` -> `VoyageSection` -> `TrendSection` -> `DashboardSection(+DNA/Milestones)`。
- 页面叙事逻辑从单纯的“数据展示”升级为“今天的你 -> 你的长期资产 -> 你的旅程隐喻 -> 你的阶段变化 -> 你的单次训练细节”。
