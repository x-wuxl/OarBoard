...
### 3.3 数据流变更说明
- 当前系统缓存逻辑（`.cache/moke`）仍处于“只加不减”的强缓存阻塞状态。如果未部署 TTL 或人为删掉文件夹，将导致昨日甚至更远历史补录的数据失去捕捉，这是一项明确需在 V3 或后期架构排期的痛点。
...
### 6.1 缓存过期死锁
- 这是目前系统数据流最大的功能盲区：`cache-service.ts` 的 `getCachedWorkoutArtifacts` 会长久信赖 `summary.json`。暂未设置基于时间的过期重抓策略。
